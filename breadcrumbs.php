...
if( !is_admin() ){
	if( $front_page != 0 ) { //if there is a frontpage
		if( is_front_page() ) {
			...
			if( is_home() ){
				...
				if( is_paged() ){
					...
					$str .= sprintf( __(' - Страница %d', 'blanc'), $current );
				}
				$str .= '</span></li>';
			} elseif( !is_page() ) {
		...
		if( is_home() ) {
			$str .='<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><span itemprop="title">' .__( "Главная", "blanc" );
			if( is_paged() ){
				$current = ( get_query_var('paged') ) ? get_query_var('paged') : 1;
				$str .= sprintf( __(' - Страница %d', 'blanc'), $current );
			}
			...
			$str .='<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="'. home_url('/') .'" itemprop="url"><span itemprop="title">' .__( "Главная", "blanc" ) .'</span></a></li>';
		}
	}
	if( is_attachment() ){
